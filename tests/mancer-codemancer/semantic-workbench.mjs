...
test('Lighthouse-rakennushaara säilyttää vakaan Coren',()=>{assert.equal(pkg.version,'1.20.0-lighthouse-shell.1');assert.equal(CORE_VERSION,'1.18.7');assert.match(html,/<dd>1\.18\.7<\/dd>/);assert.doesNotMatch(html,/MANCER REGISTRY \/ 1\.18\.0|ARKISTONHOITAJA 1\.17\.3|NANOMANCER 1\.17\.2|ARCHIVE CORE 1\.17\.1/);});
