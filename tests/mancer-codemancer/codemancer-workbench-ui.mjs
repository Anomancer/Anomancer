...
try{
 ...
 await pointerClick('[data-shell-route="archive"]');await waitFor(`document.body.dataset.shellSurface==='archive'`,'archive pointer route');assert.match(await value(`document.title`),/Arkisto/);await pointerClick('[data-shell-route="workspace"]');await waitFor(`document.body.dataset.shellSurface==='workspace' && !!document.querySelector('.mancer-code-workbench')`,'workspace pointer route');ok('desktopin Core-nav reagoi oikeaan pointer-klikkaukseen Arkisto → Nykyinen työ');
 await pointerClick('[data-local-section="tasks"]');await waitFor(`window.anomancerMancer?.activeSection?.()==='tasks' && !!document.querySelector('.mancer-task-board')`,'tasks pointer nav');await pointerClick('[data-local-section="code"]');await waitFor(`window.anomancerMancer?.activeSection?.()==='code' && !!document.querySelector('.mancer-code-workbench')`,'code pointer nav');ok('paikallisnavigaatio reagoi oikeaan pointer-klikkaukseen');
 ...
 await pointerClick('#coreSettingsButton');await waitFor(`document.querySelector('#coreSettingsDialog')?.open===true`,'settings open');await pointerClick('#coreSettingsClose');await waitFor(`document.querySelector('#coreSettingsDialog')?.open===false`,'settings close');ok('Asetukset-dialogin avaus ja sulku toimivat oikealla pointerilla');
 ...
 await send('Emulation.setDeviceMetricsOverride',{width:360,height:800,deviceScaleFactor:1,mobile:true},sessionId);await value(`window.anomancerMancer.selectSection('code')`);await waitFor(`!!document.querySelector('.mancer-code-workbench')`,'mobile code');const mobile=await value(`(()=>{const w=document.querySelector('.mancer-code-workbench'),e=document.querySelector('.mancer-code-editor textarea'),path=document.querySelector('[data-mancer-path=\"path\"]'),nav=[...document.querySelectorAll('.core-shell-nav button')];const ps=getComputedStyle(path);return{cols:getComputedStyle(w).gridTemplateColumns,overflow:document.documentElement.scrollWidth-innerWidth,font:getComputedStyle(e).fontSize,indexDisplay:getComputedStyle(document.querySelector('.mancer-item-index')).display,pathBg:ps.backgroundColor,pathBorder:ps.borderTopColor,navText:nav.map(x=>getComputedStyle(x,'::after').content),navRight:Math.max(...nav.map(x=>x.getBoundingClientRect().right))}})()`);assert.equal(mobile.cols.split(' ').length,1);assert.ok(mobile.overflow<=1,`overflow ${mobile.overflow}`);assert.ok(parseFloat(mobile.font)>=16);assert.equal(mobile.indexDisplay,'flex');assert.notEqual(mobile.pathBg,'rgba(0, 0, 0, 0)');assert.ok(mobile.navRight<=360.5,`nav right ${mobile.navRight}`);ok('360×800 reflowaa yhteen työpalstaan, tokenoi kontrollit ja pitää Core-navin viewportissa');
