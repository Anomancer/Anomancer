...
artifact.data.release={version:'1.18.7',check:'passing',notes:'Codemancer Workbench vertical slice',humanApproval:'approved'};
...
async function pointerClick(selector){
 const rect=await value(`(async()=>{const e=document.querySelector(${JSON.stringify(selector)});if(!e)return null;e.scrollIntoView({block:'center',inline:'center',behavior:'instant'});await new Promise(r=>requestAnimationFrame(()=>requestAnimationFrame(r)));const b=e.getBoundingClientRect(),x=b.left+b.width/2,y=b.top+b.height/2,hit=document.elementFromPoint(x,y);return{x,y,visible:b.width>0&&b.height>0,disabled:!!e.disabled,hit:hit?.outerHTML?.slice(0,180)||'',owns:hit===e||e.contains(hit)}})()`);
 assert.ok(rect&&rect.visible,`pointer target missing: ${selector}`);assert.equal(rect.disabled,false,`pointer target disabled: ${selector}`);assert.equal(rect.owns,true,`pointer target blocked: ${selector} hit=${rect.hit}`);
...
try{
 ...
 await pointerClick('#lighthouseMenuButton');await waitFor(`document.querySelector('#lighthouseMenuDialog')?.open===true`,'menu opens for archive');await pointerClick('[data-shell-route="archive"]');await waitFor(`document.body.dataset.shellSurface==='archive' && document.querySelector('#lighthouseMenuDialog')?.open===false`,'archive pointer route');assert.match(await value(`document.title`),/Arkisto/);await pointerClick('#lighthouseMenuButton');await waitFor(`document.querySelector('#lighthouseMenuDialog')?.open===true`,'menu opens for workspace');await pointerClick('#lighthouseMenuDialog [data-shell-route="workspace"]');await waitFor(`document.body.dataset.shellSurface==='workspace' && document.querySelector('#lighthouseMenuDialog')?.open===false && !!document.querySelector('.mancer-code-workbench')`,'workspace pointer route');ok('desktopin Lighthouse-valikko reagoi oikeaan pointer-klikkaukseen Arkisto → Nykyinen työ');
 await pointerClick('#lighthouseMenuButton');await waitFor(`document.querySelector('#lighthouseMenuDialog')?.open===true && !document.querySelector('#lighthouseWorkspaceTools')?.hidden`,'workspace tools menu');await pointerClick('[data-menu-local-section="tasks"]');await waitFor(`window.anomancerMancer?.activeSection?.()==='tasks' && !!document.querySelector('.mancer-task-board')`,'tasks pointer nav');await pointerClick('#lighthouseMenuButton');await waitFor(`document.querySelector('#lighthouseMenuDialog')?.open===true`,'workspace tools menu reopen');await pointerClick('[data-menu-local-section="code"]');await waitFor(`window.anomancerMancer?.activeSection?.()==='code' && !!document.querySelector('.mancer-code-workbench')`,'code pointer nav');ok('Valikon kontekstuaaliset työkalut vaihtavat Nykyisen työn osiota oikealla pointer-klikkauksella');
 const beforeAdd=await value(`document.querySelectorAll('.mancer-item-index button').length`);await pointerClick('[data-mancer-add="code"]');await waitFor(`document.querySelectorAll('.mancer-item-index button').length===${beforeAdd+1}`,'pointer add file');await pointerClick('.mancer-focus-head [data-mancer-remove="code"]');await waitFor(`!!document.querySelector('[data-mancer-undo]')`,'pointer remove file');await pointerClick('[data-mancer-undo]');await waitFor(`document.querySelectorAll('.mancer-item-index button').length===${beforeAdd+1}`,'pointer undo file');ok('Lisää, Poista ja Kumoa toimivat oikeilla pointer-tapahtumilla');
 await pointerClick('#lighthouseMenuButton');await waitFor(`document.querySelector('#lighthouseMenuDialog')?.open===true`,'menu opens for settings');await pointerClick('#coreSettingsButton');await waitFor(`document.body.dataset.shellSurface==='settings' && document.querySelector('[data-shell-view="settings"]')?.hidden===false && document.querySelector('#lighthouseMenuDialog')?.open===false`,'settings page open');await pointerClick('#lighthouseMenuButton');await waitFor(`document.querySelector('#lighthouseMenuDialog')?.open===true`,'menu reopens from settings');await pointerClick('#lighthouseMenuDialog [data-shell-route="dashboard"]');await waitFor(`document.body.dataset.shellSurface==='dashboard' && document.querySelector('[data-shell-view="settings"]')?.hidden===true`,'settings page close');ok('Asetussivun avaus ja paluu toimivat Valikon kautta oikealla pointerilla');
 ...
 await send('Emulation.setDeviceMetricsOverride',{width:360,height:800,deviceScaleFactor:1,mobile:true},sessionId);await value(`window.anomancerMancer.selectSection('code')`);await waitFor(`!!document.querySelector('.mancer-code-workbench')`,'mobile code');const mobile=await value(`(()=>{const w=document.querySelector('.mancer-code-workbench'),e=document.querySelector('.mancer-code-editor textarea'),path=document.querySelector('[data-mancer-path=\"path\"]'),trigger=document.querySelector('#lighthouseMenuButton');const ps=getComputedStyle(path),lefts=[...w.children].map(x=>Math.round(x.getBoundingClientRect().left));return{cols:getComputedStyle(w).gridTemplateColumns,overflow:document.documentElement.scrollWidth-innerWidth,font:getComputedStyle(e).fontSize,indexDisplay:getComputedStyle(document.querySelector('.mancer-item-index')).display,pathBg:ps.backgroundColor,pathBorder:ps.borderTopColor,gridLefts:lefts,menuRight:trigger?.getBoundingClientRect().right??Infinity}})()`);assert.equal(new Set(mobile.gridLefts).size,1,`grid columns ${mobile.cols} / lefts ${mobile.gridLefts}`);assert.ok(mobile.overflow<=1,`overflow ${mobile.overflow}`);assert.ok(parseFloat(mobile.font)>=16);assert.equal(mobile.indexDisplay,'flex');assert.notEqual(mobile.pathBg,'rgba(0, 0, 0, 0)');assert.ok(mobile.menuRight<=360.5,`menu right ${mobile.menuRight}`);ok('360×800 reflowaa yhteen työpalstaan, tokenoi kontrollit ja pitää Lighthouse-headerin viewportissa');
...
console.log(`\n${passed}/${passed} CODEMANCER WORKBENCH 1.18.4 browser checks passed.`);
