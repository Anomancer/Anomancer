...
test('mobiilin toimintorivi ei duplikoi Esikatselua eikä pysyvää VALMIS-tilaa',()=>{
  assert.match(shell,/mobileActionStrip\.innerHTML=`<button type="button" data-mobile-command="save">Tallenna<\/button>/);
  const block=shell.match(/function renderMobileActionStrip\(\)\{[\s\S]*?\n\}/)?.[0]||'';
  assert.doesNotMatch(block,/mobile-action-state/);
  assert.doesNotMatch(block,/data-mobile-command="preview"/);
});
