...
await test('1.18.7 lukitsee live-operaatiot eksplisiittiseen repository-allowlistiin',async()=>{
...
await test('1.18.7 paketti ja Codemancer Package Spec ovat päivittyneet',async()=>{
  ...
  assert.equal(pkg.version,'1.18.7');assert.equal(manifest.version,'1.3.0');assert.match(pkg.scripts.check,/tests\/release-gate\.mjs/);assert.match(gate,/tests\/operations-release\/live-path-verification\.mjs/);
});
