...
await test('Lighthouse-prerelease ja Codemancer Package Spec ovat yhtenäiset',async()=>{
  ...
  assert.equal(pkg.version,'1.22.0-lighthouse-actuator.1');assert.equal(manifest.version,'1.3.0');assert.match(pkg.scripts.check,/tests\/release-gate\.mjs/);assert.match(gate,/tests\/operations-release\/live-path-verification\.mjs/);
});
