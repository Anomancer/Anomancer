...
assert.equal(tracked.status,0,'git ls-files epäonnistui');
...
for(const [rel,hash] of before)assert.equal(fingerprint(rel),hash,`build muutti tracked sourcea: ${rel}`);
...
console.log('✓ BUILD/SOURCE BOUNDARY · tracked source immutable · output only public/');
