...
const finalizeRoot=fs.mkdtempSync(path.join(os.tmpdir(),'anomancer-vercel-output-'));
try{
  const tempPublic=path.join(finalizeRoot,'public');
  for(const rel of ['lahetykset.html','dispatches.html','lahetykset/vanha.html','dispatches/old.html','index.html']){
    const file=path.join(tempPublic,rel);fs.mkdirSync(path.dirname(file),{recursive:true});fs.writeFileSync(file,rel);
  }
  const removed=finalizeVercelOutput(tempPublic);
  assert.deepEqual(removed,['lahetykset.html','dispatches.html','lahetykset','dispatches']);
  assert.equal(fs.existsSync(path.join(tempPublic,'lahetykset.html')),false);
  assert.equal(fs.existsSync(path.join(tempPublic,'lahetykset')),false);
  assert.equal(fs.existsSync(path.join(tempPublic,'dispatches.html')),false);
  assert.equal(fs.existsSync(path.join(tempPublic,'dispatches')),false);
  assert.equal(fs.existsSync(path.join(tempPublic,'index.html')),true);
}finally{fs.rmSync(finalizeRoot,{recursive:true,force:true});}
