...
import {projectSourceStatus} from '../../server/project-source.js';

const html=fs.readFileSync('app/lighthouse/lab.html','utf8');
const js=fs.readFileSync('app/lighthouse/lab.js','utf8');

const sourceStatus=projectSourceStatus({ANOMANCER_PROJECT_WRITE:'1'});
assert.equal(sourceStatus.readable,true);
assert.equal(sourceStatus.writable,true);
...
assert.match(software.recommendation.dataNotice,/paikallisesta lähdepuusta/i);
...
const writeIntent=previewIntent({text:'Korjaa core/intent/intent-service.js ja kirjoita muutos projektiin.'},{availability:{'repository.read':true,'repository.propose':true,'repository.write':true}});
