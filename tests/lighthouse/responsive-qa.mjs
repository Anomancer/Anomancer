...
assert.match(html,/id="lighthouseLoginDialog"/);
assert.match(html,/id="lighthousePassword"/);
assert.match(js,/fetch\('\/api\/admin\/auth\?resource=login'/);
assert.match(js,/credentials:'same-origin'/);
assert.match(js,/lighthouseLoginDialog\.showModal\(\)/);
assert.match(js,/await labRequestHeaders\(\)/);
assert.doesNotMatch(js,/window\.location\.assign\('\/admin\.html\?return=%2Flab'\)/);
assert.match(admin,/ADMIN_RETURN_TARGETS=new Set\(\['\/lighthouse','\/lab'\]\)/);
