...
const original=Object.fromEntries(
  ['VERCEL_ENV','NODE_ENV','ANOMANCER_LIGHTHOUSE_LAB','ADMIN_SESSION_SECRET']
...
try{
  ...
  assert.equal(
    lighthouseLabAllowed({VERCEL_ENV:'preview',ANOMANCER_LIGHTHOUSE_LAB:'1'}),
    true
  );
  ...
  assert.equal(
    vercel.git?.deploymentEnabled?.['architecture/lighthouse-v1'],
    false
  );
  assert.ok(vercel.headers.some(item=>item.source==='/api/lab/(.*)'));

  console.log('✓ Lighthouse Lab API boundary · remote lock, auth, CSRF, origin and size limits');
}finally{
