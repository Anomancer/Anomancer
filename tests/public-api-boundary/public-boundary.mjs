...
await test('julkinen snapshot ei sisällä tunnettuja salaisuusnimiä tai tarkkoja tokenkattoja',()=>{const raw=JSON.stringify(core);for(const marker of ['ADMIN_SESSION_SECRET','DEEPSEEK_API_KEY','BLOB_READ_WRITE_TOKEN','RESEND_API_KEY','24000','48000'])assert.doesNotMatch(raw,new RegExp(marker));});
...
await test('Core-sivu sanoo eksplisiittisesti julkisen ja yksityisen rajan ja linkittää julkaisukuitin',()=>{const fi=fs.readFileSync('core.html','utf8'),en=fs.readFileSync('core-en.html','utf8');assert.match(fi,/Oikeat ajot, kehotteet, mallikohteet, työtilojen sisältö ja ohjaus pysyvät yksityisinä/i);assert.match(en,/Real runs, prompts, model targets, workspace content and control stay private/i);for(const html of [fi,en]){assert.match(html,/release-provenance\.json/);assert.doesNotMatch(html,/16[ ,.]?000 (?:tulostokenia|output tokens)/i);}});
await test('release provenance sisältää vain turvallisen hash-jäljen ja disclosure-booleanit',()=>{const p=createReleaseProvenance({publicCore:core,apiFunctionCount:4,builtAt:'2026-08-27T00:00:00.000Z'});assert.equal(p.apiSurface.functionCount,4);assert.match(p.publicSchemaHash,/^[a-f0-9]{64}$/);assert.match(p.registryHashes.agents,/^[a-f0-9]{64}$/);assert.equal(p.disclosureBoundary.rawPrompts,false);assert.equal(p.disclosureBoundary.rawOutputs,false);assert.equal(p.disclosureBoundary.runtimeProfiles,false);const raw=JSON.stringify(p);for(const marker of ['ADMIN_SESSION_SECRET','DEEPSEEK_API_KEY','BLOB_READ_WRITE_TOKEN','RESEND_API_KEY'])assert.doesNotMatch(raw,new RegExp(marker,'i'));});
await test('public disclosure -politiikka on repo-dokumenttina mutta private IP lineage ei ole',()=>{assert.equal(fs.existsSync('docs/architecture/boundaries-provenance.md'),true);assert.equal(fs.existsSync('ANOMANCER_IP_PRIVATE.md'),false);});
