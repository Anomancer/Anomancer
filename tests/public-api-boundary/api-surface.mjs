...
await test('Vercel API-pinta sisältää neljä vakaata entrypointtia ja lukitun Lighthouse Labin',()=>{assert.deepEqual(allJs('api'),['api/admin/auth.js','api/admin/content.js','api/admin/core.js','api/contact.js','api/lab/intent.js']);const lab=fs.readFileSync('api/lab/intent.js','utf8');assert.match(lab,/lighthouseLabAllowed/);assert.match(lab,/lighthouseLabRequiresAuth/);assert.match(lab,/sameOrigin/);assert.match(lab,/requireCsrf/);assert.match(lab,/readJson\(req,MAX_BODY_BYTES\)/);});
