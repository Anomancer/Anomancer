...
await test('Vercel API-pinta sisältää vakaat gatewayt ja kaksi lukittua Lighthouse Lab -reittiä',()=>{assert.deepEqual(allJs('api'),['api/admin/auth.js','api/admin/content.js','api/admin/core.js','api/contact.js','api/lab/intent.js','api/lab/preview.js']);for(const file of ['api/lab/intent.js','api/lab/preview.js']){const lab=fs.readFileSync(file,'utf8');assert.match(lab,/lighthouseLabAllowed/);assert.match(lab,/lighthouseLabRequiresAuth/);assert.match(lab,/sameOrigin/);assert.match(lab,/requireCsrf/);assert.match(lab,/readJson\(req,MAX_BODY_BYTES\)/);}const preview=fs.readFileSync('api/lab/preview.js','utf8');assert.match(preview,/previewIntent/);assert.doesNotMatch(preview,/runIntent/);});
