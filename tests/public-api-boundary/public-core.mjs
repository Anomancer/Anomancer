...
test('/lighthouse pysyy yksityisenä ja /admin on vain alias',()=>{
  const lighthouse=vercel.headers.find(h=>h.source==='/lighthouse');
  assert.ok(lighthouse);assert.match(JSON.stringify(lighthouse),/noindex/);assert.match(JSON.stringify(lighthouse),/no-store/);
  assert.ok(vercel.redirects.some(r=>r.source==='/admin'&&r.destination==='/lighthouse/workbench'));
});
...
test('public Core kertoo julkisen ja yksityisen rajan',()=>{assert.match(html,/Lasiseinä ei ole avoin ovi/);assert.match(html,/href="\/lighthouse\/login"/);assert.match(html,/Oikeat ajot, kehotteet, mallikohteet, työtilojen sisältö ja ohjaus pysyvät yksityisinä/);});
