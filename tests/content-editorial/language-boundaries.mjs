...
const coreJs=read('core-public.js'),coreRenderer=read('public-core-render.js'),build=read('scripts/build-blog.mjs');
...
test('dynaaminen Core UI valitsee sanaston dokumentin lang-attribuutista',()=>{assert.match(coreJs,/documentElement\.lang/);assert.match(coreJs,/publicCoreLang/);assert.match(coreRenderer,/const COPY=/);assert.match(coreRenderer,/fi:/);assert.match(coreRenderer,/en:/);});
test('build erottaa FI- ja EN-Core-reitit',()=>{assert.match(build,/corePath: '\/core'/);assert.match(build,/corePath: '\/en\/core'/);assert.match(build,/\['\/en\/core',null\]/);assert.match(build,/core-en\.html/);assert.match(build,/write\('en\/core\.html',renderPublicCoreFallback/);});
