...
test('rakennushaaran metadata ja check-portti ovat yhtenäiset',()=>{assert.equal(pkg.version,'1.22.0-lighthouse-actuator.1');assert.match(pkg.scripts.check,/tests\/release-gate\.mjs/);assert.match(gate,/tests\/content-editorial\/evidence-interaction\.mjs/);assert.match(html,/id="systemCoreVersion">1\.18\.7/);});
