...
test('lähteet lisätään tarkistamattomina ja kuitataan yhdellä ihmisen vahvistuksella',()=>{assert.match(html,/id="sourceComposerTitle"/);assert.match(html,/id="sourceComposerUrl"/);assert.match(html,/id="sourceAddBtn"/);assert.match(js,/function addSourceFromComposer\(/);assert.match(js,/origin:'human',verification:'candidate'/);assert.match(js,/Merkitse lähde tarkistetuksi/);assert.match(js,/Erillistä selvitystä ei tarvitse kirjoittaa/);assert.doesNotMatch(js,/label:'Mitä tarkistit\?'/);for(const field of ['verifiedBy','verifiedAt','verificationMethod','verificationEvidence','verificationNotes'])assert.match(js,new RegExp(field));});
...
test('rakennushaaran metadata ja check-portti ovat yhtenäiset',()=>{assert.match(pkg.version,/^1\.26\./);assert.match(pkg.scripts.check,/tests\/release-gate\.mjs/);assert.match(gate,/tests\/content-editorial\/evidence-interaction\.mjs/);assert.match(html,/<dt>Lighthouse<\/dt><dd>1\.26\.\d+<\/dd>/);assert.match(html,/id="systemCoreVersion">—<\/dd>/);});
