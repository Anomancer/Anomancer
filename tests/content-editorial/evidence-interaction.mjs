...
test('release metadata ja check-portti ovat 16.8.4',()=>{assert.equal(pkg.version,'1.18.6');assert.match(pkg.scripts.check,/tests\/release-gate\.mjs/);assert.match(gate,/tests\/content-editorial\/evidence-interaction\.mjs/);assert.match(html,/id="systemCoreVersion">1\.18\.6/);});
