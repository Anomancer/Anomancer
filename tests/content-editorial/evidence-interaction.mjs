...
test('rakennushaaran metadata ja check-portti ovat yhtenäiset',()=>{assert.match(pkg.version,/^1\.25\./);assert.match(pkg.scripts.check,/tests\/release-gate\.mjs/);assert.match(gate,/tests\/content-editorial\/evidence-interaction\.mjs/);assert.match(html,/<dt>Lighthouse<\/dt><dd>1\.25\.\d+<\/dd>/);assert.match(html,/id="systemCoreVersion">—<\/dd>/);});
