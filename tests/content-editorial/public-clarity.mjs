...
test('julkinen Core säilyy vakaana Lighthouse-rakennushaarassa',()=>{assert.equal(pkg.version,'1.22.0-lighthouse-actuator.1');assert.equal(publicCore.version,'1.18.7');});
...
test('julkinen Core ei nimeä itseään ohjaustasoksi ja Home/Core tarjoavat Admin-reitin footerissa',()=>{assert.doesNotMatch(fiCore,/Agenttien ohjaustaso|<p class="eyebrow">Ohjaustaso<\/p>/);assert.doesNotMatch(enCore,/Agent Control Plane|<p class="eyebrow">Control plane<\/p>/);assert.match(fiCore,/Julkinen rakennenäkymä/);assert.match(enCore,/Public Architecture View/);for(const html of [fiCore,enCore]){assert.equal((html.match(/href="\/admin"/g)||[]).length,2);assert.match(html,/footer-admin-link/);}for(const html of [fiHome,enHome])assert.match(html,/class="footer-admin-link" href="\/admin">Admin<\/a>/);});
