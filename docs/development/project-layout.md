# Project layout and generated-output boundary

The project directory separates durable source from disposable build products.
...
└── public/              # GENERATED, disposable build output
```

## Generated root products

The following are also disposable build outputs:
...
`npm run check` bootstraps the build first, so a clean project directory does not require generated output to be carried between releases.

## Transitional source/build hybrid

`index.html`, `en.html`, `core.html` and `core-en.html` remain tracked source files that the build synchronizes in-place before staging. Separating these into immutable templates and generated output is a later refactor and is intentionally not mixed into this cleanup.
