...
function renderCore(core={}){
  ...
  $('#corePolicy').replaceChildren(
    coreStatus(
      'Lighthouse käytettävissä tässä ympäristössä',
      (environment.appAllowed??environment.labAllowed)===true,
      environment.productionDefaultLocked
        ?'Lighthouse on tässä ympäristössä poistettu eksplisiittisellä sovelluspolitiikalla.'
        :'Lighthouse on käytettävissä. Etäympäristössä yksityinen pinta vaatii kirjautumisen.'
    ),
    coreStatus(
    ...
    coreStatus(
      ...
      'Lighthouse ei julkaise sisältöä automaattisesti.'
    ),
    coreStatus(
  ...
  const traces=[
    ...
    ['D4 · Orkesteri',completeness.orchestration],
...
function renderMutation(mutation={}){
  ...
  panel.hidden=false;
  $('#mutationSummary').textContent=proposal.summary||'Rajattu projektimuutos odottaa tarkistusta.';
  ...
  if(mutation.available===true&&approval?.token&&approval?.confirmationPhrase){
    ...
    $('#mutationBoundary').textContent=`Kohde: ${proposal.base?.project||'Anomancer'} @ ${String(proposal.base?.revision||'').slice(0,12)}. Hyväksyntä kirjoittaa vain yllä näkyvät projektitiedostot paikalliseen kehityskansioon. Tuotantoruntime ei saa kirjoittaa lähdekoodia.`;
    $('#mutationExecute').disabled=false;
    message.textContent='Mitään projektitiedostoa ei ole vielä muutettu.';
  }else{
...
async function executeCurrentMutation(){
  ...
  try{
    ...
    message.textContent='Hyväksytyt projektitiedostot kirjoitettiin paikalliseen kehityskansioon.';
    $('#mutationConfirmation').disabled=true;
    const receipt=payload.receipt||{};
    const title=document.createElement('strong');title.textContent='Suorituskuitti';
    const meta=document.createElement('p');meta.textContent=[`id ${receipt.id||''}`,`revision ${String(receipt.execution?.revision||'').slice(0,12)}`,`receipt ${String(receipt.receiptHash||'').slice(0,12)}`].filter(Boolean).join(' · ');
...
function renderResult(payload,{initial=false}={}){
  ...
  const handoff=$('#workbenchHandoff');
  if(handoff){
    const url=new URL('/lighthouse/workbench',location.origin);
    if(activeWorkspace?.id)url.searchParams.set('lightWork',activeWorkspace.id);
    url.searchParams.set('workspace','default');
    url.searchParams.set('view','workspace');
    handoff.href=`${url.pathname}${url.search}`;
  }
...
function setupVoiceInput(){
  ...
  voiceInput.addEventListener('click',()=>{
    ...
    voiceInput.querySelector('[data-voice-label]').textContent='Kuuntelen…';
    ...
    recognition.addEventListener('end',()=>{
      ...
      voiceInput.querySelector('[data-voice-label]').textContent='Puhu';
...
function showResponsiveDepth(id,{focus=false}={}){
  ...
  depthInspectorTitle.textContent=meta.label;
