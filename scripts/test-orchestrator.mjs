...
test('adminissa on 14.1.1 Orchestrator ja live terminal',()=>{assert.match(html,/14\.1\.1 · ORCHESTRATOR RESILIENCE/);assert.match(html,/orchestraTerminal/);assert.match(html,/HUMAN FINAL AUTHORITY/);});
...
test('tilapäinen DeepSeek-virhe saa yhden automaattisen retryn',()=>{assert.match(js,/automaattinen retry 1\/1/);assert.match(js,/attempt===1&&isRetryable/);assert.match(js,/DEEPSEEK_EMPTY/);assert.match(js,/\[502,504\]/);});
test('checkpoint tallennetaan vain selaimen sessionStorageen',()=>{assert.match(js,/sessionStorage\.setItem\(CHECKPOINT_KEY/);assert.match(js,/sessionStorage\.getItem\(CHECKPOINT_KEY/);assert.match(js,/checkpoint\.v14\.1\.1/);assert.doesNotMatch(js,/localStorage\.setItem/);});
