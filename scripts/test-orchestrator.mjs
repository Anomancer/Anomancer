...
test('adminissa on 14.2 identiteettiturvallinen Orchestrator ja live terminal',()=>{assert.match(html,/14\.2 · IDENTITY-SAFE ORCHESTRATOR/);assert.match(html,/orchestraTerminal/);assert.match(html,/HUMAN FINAL AUTHORITY/);});
...
test('välitulokset siirtyvät työmuistissa seuraaville agenteille',()=>{assert.match(js,/ctx\.post\.sources=mergeSources/);assert.match(js,/ctx\.post\.claims=d\.result\.claims/);assert.match(js,/ctx\.post\.body=d\.result\.body/);assert.match(js,/RAKENNEAGENTIN EHDOTUS JSON/);assert.match(js,/KRIITIKON HAVAINNOT JSON/);});
...
test('checkpoint tallennetaan vain selaimen sessionStorageen',()=>{assert.match(js,/sessionStorage\.setItem\(CHECKPOINT_KEY/);assert.match(js,/sessionStorage\.getItem\(CHECKPOINT_KEY/);assert.match(js,/checkpoint\.v14\.2\.0/);assert.doesNotMatch(js,/localStorage\.setItem/);});
test('checkpoint on sidottu luonnoksen identiteettiin ja valmis tulos säilyy',()=>{assert.match(js,/draftIdentity/);assert.match(js,/sameDocument/);assert.match(js,/ctx\.status='complete'/);assert.match(js,/function finishRun[\s\S]{0,1200}saveCheckpoint\(ctx\)/);});
