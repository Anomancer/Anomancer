...
import {readLighthouseCss} from './read-lighthouse-css.mjs';
...
for(const name of ['lab.js','workspace-store.js']){
  fs.copyFileSync(path.join(SOURCE,name),path.join(TARGET,name));
}
fs.writeFileSync(path.join(TARGET,'lab.css'),readLighthouseCss(ROOT));
fs.rmSync(path.join(TARGET,'styles'),{recursive:true,force:true});
