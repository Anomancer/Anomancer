...
await test('julkinen Core-selain ei kutsu admin-API:a eikä renderöi runtime-budjetteja',()=>{const js=fs.readFileSync('core-public.js','utf8');assert.doesNotMatch(js,/\/api\/admin\//);assert.doesNotMatch(js,/maxOutputTokens|allowedTargets|defaultTarget/);assert.match(js,/providerPrivate/);});
await test('Core-sivu sanoo eksplisiittisesti julkisen kenttärajan ja linkittää julkaisukuitin',()=>{const fi=fs.readFileSync('core.html','utf8'),en=fs.readFileSync('core-en.html','utf8');assert.match(fi,/sallittujen kenttien julkinen rakennekartta/i);assert.match(en,/allowlist/i);for(const html of [fi,en]){assert.match(html,/release-provenance\.json/);assert.doesNotMatch(html,/16[ ,.]?000 (?:tulostokenia|output tokens)/i);}});
