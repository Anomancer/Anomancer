...
import { readAdminCss, ADMIN_STYLE_FILES } from './read-admin-css.mjs';
...
for(const [name,width,height,features] of matrix){
  ...
  const m=evalResp.result.value;assert.ok(m.overflow<=1,`${name}: vaakavuoto ${m.overflow}px`);assert.ok(m.minTarget>=43.5,`${name}: kosketuskohde ${m.minTarget}px`);assert.ok(m.minUI>=13.9,`${name}: UI-teksti ${m.minUI}px`);assert.ok(m.minMeta>=11.9,`${name}: metadata ${m.minMeta}px`);assert.ok(m.focusStyle!=='none'&&m.focusOutline>=1.9,`${name}: focus-ring puuttuu`);if(width<=760){assert.ok(m.dockVisible,`${name}: mobiilidokki puuttuu`);assert.equal(m.localNavVisible,false,`${name}: desktop-rail näkyy mobiilissa`);assert.ok(m.topChrome<=120,`${name}: yläkromi ${m.topChrome}px`);}else{assert.ok(m.localNavVisible,`${name}: desktop-rail puuttuu`);}
