...
function audienceButtons(lang, published){ const c=copy[lang]; const used=[...new Set(published.flatMap(p=>p.audience||['all']).filter(id=>id!=='all'))]; const primary=['entrepreneur','developer','teacher']; const visible=[...primary,...audienceOrder.filter(id=>id!=='all'&&used.includes(id)&&!primary.includes(id))]; return [`<button class="audience-filter is-active" data-audience-filter="all" aria-pressed="true">${c.all}</button>`,...visible.map(id=>`<button class="audience-filter" data-audience-filter="${id}" aria-pressed="false">${audiences[id][lang]}</button>`)].join(''); }
