...
const copy = {
  ...
  en: {
    ...
    published: 'Published', pinned: 'Pinned', minRead: 'min read', category: 'Topic', audience: 'Who is this useful for?', audienceShort:'Audience', filterOpen:'Filter', filterTitle:'Filter dispatches', clearFilters:'Clear', showResults:'Show results', related: 'More dispatches', byline: 'Written by', answer: 'Direct answer', evidence: 'Claims & evidence', sources: 'Sources', supported: 'Supported claim', interpretation: 'Interpretation', open: 'Open question',
...
function categoryButtons(lang, published){ const c=copy[lang]; const active=categoryOrder.map(id=>[id,published.filter(p=>p.category===id).length]).filter(([,count])=>count>0); return `<button class="category-filter is-active" data-category-filter="all" data-filter-label="${escAttr(c.all)}" aria-pressed="true">${c.all}<span>${published.length}</span></button>${active.map(([id,count])=>`<button class="category-filter" data-category-filter="${id}" data-filter-label="${escAttr(categories[id][lang])}" aria-pressed="false">${categories[id][lang]}<span>${count}</span></button>`).join('')}`; }
function audienceButtons(lang, published){ const c=copy[lang]; const visible=audienceOrder.filter(id=>id!=='all'&&published.some(p=>(p.audience||['all']).includes(id))); const count=id=>published.filter(p=>(p.audience||['all']).includes(id)).length; return [`<button class="audience-filter is-active" data-audience-filter="all" data-filter-label="${escAttr(c.all)}" aria-pressed="true">${c.all}<span>${published.length}</span></button>`,...visible.map(id=>`<button class="audience-filter" data-audience-filter="${id}" data-filter-label="${escAttr(audiences[id][lang])}" aria-pressed="false">${audiences[id][lang]}<span>${count(id)}</span></button>`)].join(''); }
...
function renderIndex(lang, posts){
  ...
  const audienceBar=audienceControls?`<div class="audience-bar desktop-filter-surface" aria-label="${escAttr(c.audience)}"><span>${c.audience}</span><div class="audience-filters">${audienceControls}</div></div>`:'';
  ...
  return `${pageHead({lang,title:`${c.title[0]}${c.title.slice(1).toLowerCase()} | ${SITE_NAME}`,description:c.description,url:`${SITE}${c.blogPath}`,alternates,jsonLd:blogJsonLd(lang),rss:lang==='fi'?`${SITE}/rss.xml`:`${SITE}/rss-en.xml`})}<body><a class="skip-link" href="#main-content">${skip}</a>${header(lang,true)}<main id="main-content" class="section transmission-shell blog-shell"><div class="transmission-intro"><p class="eyebrow">${c.eyebrow}</p><h1>${c.title}</h1><p class="intro">${c.intro}</p></div>${audienceBar}${mobileFilters}<div class="blog-layout"><aside class="category-sidebar desktop-filter-surface" aria-label="${escAttr(c.category)}"><div class="category-sticky"><p class="category-title">${c.category}</p>${categoryButtons(lang,pub)}</div></aside><section class="blog-stream"><div id="dispatch-empty" class="dispatch-empty" hidden>${c.empty}</div><div class="section-minihead archive-head"><span>${c.archive}</span><span id="dispatch-count">${pub.length}</span></div><div class="dispatch-grid">${pub.map(p=>postCard(p,lang)).join('')}</div></section></div></main><footer class="footer"><span>${c.footer}</span><a href="${c.home}">${c.homeBack}</a></footer>${menuScript()}</body></html>`;
}
...
function evidenceBlocks(p){
  ...
  const sourceHtml=showSourceStrip&&sources.length?`<div class="article-source-strip" aria-label="${escAttr(c.sources)}"><strong>${esc(c.sources)}</strong>${sources.map((src,i)=>`<a id="source-${i+1}" href="${escAttr(src.url)}" target="_blank" rel="noopener noreferrer"><span>${i+1}</span>${esc(src.title)}</a>`).join('')}</div>`:'';
...
function renderArticle(p,all){
  ...
  if(trans) langTargets[trans.lang]=`${copy[trans.lang].articleBase}/${trans.slug}`;
...
function syncPublicCoreFallback(rel,lang,core){
  ...
  const replacements={AGENT_COUNT:String(view.agentCount),ORCHESTRA_COUNT:String(view.orchestraCount),AGENTS:view.agentsHtml,ORCHESTRAS:view.orchestrasHtml,MODELS:view.modelsHtml,TOOLS:view.toolsHtml,WORKSPACES:view.workspacesHtml};
...
const evidenceManifest={version:'anomancer.evidence/v1',generatedAt:manifest.generatedAt,articles:published.map(p=>({articleId:`${articleUrl(p)}#article`,url:articleUrl(p),title:p.title,lang:p.lang,answer:p.answer||'',claims:p.claims||[],sources:p.sources||[],citationMode:p.citationMode||'inline',citationPlacements:p.citationPlacements||[],visualizations:p.visualizations||[]}))};
...
const publicFiles = [
  ...
  'ui-tokens.css','styles.css','core.css','admin.css','admin-shell.css','admin-workspace.css','admin-editorial.css','admin-narrative.css','admin-control-plane.css','admin-archive.css','admin-nanomancer.css','admin-mancer.css','admin-responsive.css','admin.js','admin-workspaces.js','admin-archive.js','admin-nanomancer.js','admin-mancer.js','admin-shell.js','admin-overlays.js','admin-core.js','admin-agents.js','admin-orchestras.js','admin-machine-room.js','admin-orchestrator.js','admin-narramancer.js','narramancer-export.js','lahetyskone-pwa.js','lahetyskone-sw.js','manifest.webmanifest','favicon.svg',
