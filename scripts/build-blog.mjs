...
const copy = {
    ...
    published: 'Julkaistu', pinned: 'Pinnattu', minRead: 'min lukuaika', category: 'Aihe', audience: 'Kenelle tästä on hyötyä?', related: 'Muita lähetyksiä',
  ...
  en: {
    ...
    published: 'Published', pinned: 'Pinned', minRead: 'min read', category: 'Topic', audience: 'Who is this useful for?', related: 'More dispatches',
...
function categoryButtons(lang, published){ const c=copy[lang]; const active=categoryOrder.map(id=>[id,published.filter(p=>p.category===id).length]).filter(([,count])=>count>0); return `<button class="category-filter is-active" data-category-filter="all">${c.all}<span>${published.length}</span></button>${active.map(([id,count])=>`<button class="category-filter" data-category-filter="${id}">${categories[id][lang]}<span>${count}</span></button>`).join('')}`; }
function audienceButtons(lang, published){ const c=copy[lang]; const used=[...new Set(published.flatMap(p=>p.audience||['all']).filter(id=>id!=='all'))]; if(!used.length) return ''; return [`<button class="audience-filter is-active" data-audience-filter="all">${c.all}</button>`,...audienceOrder.filter(id=>id!=='all'&&used.includes(id)).map(id=>`<button class="audience-filter" data-audience-filter="${id}">${audiences[id][lang]}</button>`)].join(''); }
...
const manifest={generatedAt:new Date().toISOString(),published:published.map(p=>({lang:p.lang,slug:p.slug,title:p.title,category:p.category,audience:p.audience||['all'],pinned:Boolean(p.pinned),date:p.date,url:articleUrl(p),coverImage:p.coverImage||''}))};
