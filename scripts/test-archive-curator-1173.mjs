...
const fs=await import('node:fs');const html=fs.readFileSync('admin.html','utf8'),route=fs.readFileSync('server/admin-routes/archive.js','utf8'),publicCore=fs.readFileSync('server/public-core.js','utf8');assert.match(html,/ARKISTONHOITAJA 1\.17\.3/);assert.match(html,/SUGGESTIONS ONLY/);assert.match(html,/Tallenna raportti Arkistoon/);assert.doesNotMatch(route,/apply-curator|curator-apply|action==='curator/i);assert.doesNotMatch(publicCore,/archive-curator|arkistonhoitaja/i);ok('UI exposes explicit human report persistence but no curator mutation endpoint or public disclosure');
