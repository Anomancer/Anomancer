...
await test('1.18.5 paketti ja Codemancer Package Spec ovat päivittyneet',async()=>{
  const pkg=JSON.parse(read('package.json')),manifest=JSON.parse(read('mancers/codemancer/manifest.json'));
  assert.equal(pkg.version,'1.18.5');assert.equal(manifest.version,'1.3.0');assert.match(pkg.scripts.check,/test-live-path-verification-1185/);
});
