...
test('Core Registry on 15.2 ja agentteja on edelleen kahdeksan',()=>{assert.equal(CORE_VERSION,'15.2.0');assert.equal(AGENT_REGISTRY.length,8);assert.equal(ORCHESTRA_REGISTRY.length,1);});
...
test('Models, Tools ja Usage johdetaan julkisesta Agent Registrystä',()=>{assert.match(js,/renderModels\(core\)/);assert.match(js,/renderTools\(core\)/);assert.match(js,/renderUsage\(core\)/);assert.match(js,/maxOutputTokens/);assert.match(js,/agent\.tools/);});
