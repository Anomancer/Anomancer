...
test('Core Registry 16.6 näyttää molemmat built-in-universumit ilman agenttien julkaisuoikeutta',()=>{assert.equal(CORE_VERSION,'16.7.0');assert.equal(AGENT_REGISTRY.length,18);assert.ok(AGENT_REGISTRY.some(a=>a.id==='visualization'));assert.ok(AGENT_REGISTRY.some(a=>a.id==='narrative-continuity'));assert.equal(ORCHESTRA_REGISTRY.length,2);assert.equal(ORCHESTRA_REGISTRY[0].steps.length,8);assert.equal(ORCHESTRA_REGISTRY[1].steps.length,9);assert.equal(ORCHESTRA_REGISTRY[0].stages.includes('visualization'),false);});
test('julkinen Core näyttää built-in Orchestra Contractit mutta ei custom-orkestereita',()=>{for(const o of ORCHESTRA_REGISTRY){assert.equal(o.source,'built-in');assert.ok(Array.isArray(o.steps));}assert.equal(ORCHESTRA_REGISTRY.find(o=>o.id==='editorial').steps.length,8);assert.equal(ORCHESTRA_REGISTRY.find(o=>o.id==='narramancer').steps.length,9);assert.doesNotMatch(js,/api\/admin\/orchestras/);});
