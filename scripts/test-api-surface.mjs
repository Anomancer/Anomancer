...
await test('admin-JS käyttää vain uusia gateway-reittejä',()=>{const files=['admin.js','admin-agents.js','admin-core.js','admin-orchestras.js','admin-orchestrator.js','admin-workspaces.js'];const raw=files.map(f=>fs.readFileSync(f,'utf8')).join('\n');for(const old of ['/api/admin/agents','/api/admin/login','/api/admin/logout','/api/admin/media','/api/admin/orchestras','/api/admin/posts','/api/admin/runs','/api/admin/runtime','/api/admin/session','/api/admin/status'])assert.equal(raw.includes(old),false,`legacy endpoint remains: ${old}`);assert.match(raw,/\/api\/admin\/auth\?resource=session/);assert.match(raw,/\/api\/admin\/content\?resource=posts/);assert.match(raw,/\/api\/admin\/core\?resource=agents/);assert.match(raw,/\/api\/admin\/core\?resource=runtime/);assert.match(raw,/\/api\/admin\/core\?resource=runs/);});
