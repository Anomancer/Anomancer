...
const output=rel=>path.join(PUBLIC,rel);
...
for(const rel of generated){
  const s=fs.readFileSync(output(rel),'utf8');
  if(s.includes(OLD)) failures.push(`${rel}: vanha vercel.app jäi outputtiin`);
}
const manifest=JSON.parse(fs.readFileSync(output('content-manifest.json'),'utf8'));
...
for(const item of fi){
  ...
  if(!audience.length){failures.push(`${item.slug}: audience puuttuu`);continue;}
...
const list=fs.readFileSync(output('lahetykset.html'),'utf8');
