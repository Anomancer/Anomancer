...
const fsm=await import('node:fs');const html=fsm.readFileSync('admin.html','utf8'),build=fsm.readFileSync('scripts/build-blog.mjs','utf8');assert.match(html,/CAPABILITY PLUGIN \/ NANOMANCER 1\.17\.2/);assert.match(html,/admin-nanomancer\.js/);assert.match(build,/admin-nanomancer\.js/);assert.match(build,/admin-nanomancer\.css/);ok('Nanomancer Workbench and production build assets are wired into the private Core');
