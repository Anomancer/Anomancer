...
const admin=read('admin-orchestrator.js'), adminCss=read('admin-control-plane.css'), coreJs=read('core-public.js'), coreCss=read('core.css'), fi=read('core.html'), en=read('core-en.html'), build=read('scripts/build-blog.mjs'), site=read('site.js'), css=read('styles.css'), domain=read('scripts/domain-migration-check.mjs');
...
test('julkinen Core välittää vaiheiden määrän DOMiin',()=>assert.match(coreJs,/data-step-count="\$\{steps\.length\}"/));
