...
import {getProjectFile,projectSourceStatus} from './project-source.js';
...
export function capabilityAvailability(env=process.env){
  const project=projectSourceStatus(env);
  const mutationReady=project.writable;
  const availability={
    'research.search':Boolean(env.BRAVE_SEARCH_API_KEY),
    'repository.read':project.readable,
...
export async function executeLighthouseHands({intent={},route={},capabilityRoute={},taskGraph=null}={}){
  ...
  const repository=projectSourceStatus();
  const events=[];
  const context=[];
  const sources=[];
  const tools=[];
