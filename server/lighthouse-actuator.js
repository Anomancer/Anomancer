...
import {getProjectFile,getProjectHead,projectSourceStatus,writeProjectFiles} from './project-source.js';
...
const SECRET_VALUE=/(?:sk-[A-Za-z0-9_-]{20,}|-----BEGIN [A-Z ]*PRIVATE KEY-----|(?:token|secret|password)\s*[:=]\s*["'][^"']{16,}["'])/i;
const DENIED_PATH=/(^|\/)(?:\.git|\.vercel|node_modules|scripts\/governance)(?:\/|$)|(^|\/)\.env(?:\.|$)|(^|\/)(?:id_rsa|id_ed25519|credentials?|secrets?)(?:\.|$)|^(?:vercel\.json|package\.json|package-lock\.json|server\/auth\.js|server\/lighthouse-actuator\.js|api\/lab\/mutation\.js)$/i;
...
export function mutationRuntimeStatus(){
  const project=projectSourceStatus();
  return {format:'anomancer-mutation-runtime-status/v2',available:project.writable&&String(process.env.ADMIN_SESSION_SECRET||'').length>=32,project,productionWrite:false,executionTarget:'local-project-files'};
}
...
export async function sealLighthouseMutation(proposal,{session,adapters={}}={}){
  ...
  const head=await (adapters.getProjectHead||adapters.getRepositoryHead||getProjectHead)();
  const bound=[];
  for(const file of files){
    const current=await (adapters.getFile||getProjectFile)(file.path);
  ...
  const id=approvalId();
  const normalized={format:MUTATION_PROPOSAL_FORMAT,summary:clean(proposal.summary,2400),risk:proposal.risk||'high',verification:Array.isArray(proposal.verification)?proposal.verification.slice(0,6):[],files:bound,totalBytes:bound.reduce((sum,item)=>sum+item.bytes,0),base:{project:head.project||head.repo||'Anomancer',revision:head.sha}};
  const proposalHash=sha(normalized),expiresAt=new Date(Date.now()+TTL_MS).toISOString();
  const payload={v:3,id,proposalHash,baseRevision:head.sha,sessionHash:sh,exp:Date.now()+TTL_MS};
  return {proposal:normalized,approval:{format:MUTATION_APPROVAL_FORMAT,id,proposalHash,confirmationPhrase:confirmationFor(id),expiresAt,token:sign(payload),humanApprovalRequired:true,productionWrite:false,executionTarget:'local-project-files'}};
}
...
export async function executeLighthouseMutation({proposal,approval,confirmation,session,adapters={}}={}){
  ...
  if(token.v!==3)fail('Mutation approval on vanhaa toteutusmallia. Tee uusi ehdotus.','LIGHTHOUSE_MUTATION_TOKEN_VERSION',409);
  ...
  if(token.baseRevision!==proposal?.base?.revision)fail('Mutation target muuttui hyväksynnän jälkeen.','LIGHTHOUSE_MUTATION_TARGET_MISMATCH',409);
  for(const file of files){
    const current=await (adapters.getFile||getProjectFile)(file.path);
  ...
  const execution=await (adapters.writeFiles||writeProjectFiles)({files:files.map(({path,content})=>({path,content}))});
  const receipt={format:MUTATION_RECEIPT_FORMAT,id:token.id,proposalHash,approvedBy:sh.slice(0,16),executedAt,confirmationMatched:true,productionWrite:false,externalSideEffect:'local-project-files-written',execution:{project:execution.project||proposal.base.project,mode:execution.mode||'local-project',files:execution.files||files.map(item=>item.path),revision:execution.revision||sha(files.map(item=>item.afterHash).join(':'))}};
