...
export function promptFor(agent,post,custom=''){
  ...
  if(agent==='critic') return {
    ...
    user:`Critique the draft, including whether the selected audience and depth are actually served without sacrificing accuracy. Return JSON with keys verdict, issues, strengths, questions. issues is an array of {severity,type,excerpt,problem,fix}; severity is high, medium or low. Return at most 12 distinct issues, at most 8 strengths and at most 8 questions. Keep excerpt, problem and fix concise and avoid repeating the same concern in multiple fields. Do not rewrite the whole article.${context}`
  };
