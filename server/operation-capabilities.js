...
export async function planCapabilityOperation({kind,workspace,session,sourceOperationId='',rollbackTarget=''}={}){
  ...
  if(kind==='repository.write'){
    ...
    const source=requireSource(await getOperation(sourceOperationId,{workspaceId:workspace.id,force:true}),'repository.write',['succeeded']);plan={sourceOperationId:source.id,repo:source.plan.repo,sourceRef:source.execution.branch,commitSha:source.execution.commitSha,workflowMode:'tests'};
  }else if(kind==='git.pull-request'||kind==='deploy.preview'){
    const source=requireSource(await getOperation(sourceOperationId,{workspaceId:workspace.id,force:true}),'tests.run',['passed']);plan={sourceOperationId:source.id,repositoryOperationId:source.plan.sourceOperationId,repo:source.plan.repo,sourceRef:source.plan.sourceRef,commitSha:source.plan.commitSha,workflowMode:kind==='deploy.preview'?'preview':null};
...
function successStatus(kind){if(kind==='repository.write')return'succeeded';if(kind==='git.pull-request')return'open';if(kind==='repository.rollback'||kind==='deploy.rollback')return kind==='repository.rollback'?'rolled_back':'dispatched';return'dispatched';}
...
export async function refreshCapabilityOperation({operationId,expectedRevision,workspace,session}={}){
  ...
  if(['tests.run','deploy.preview','deploy.production','deploy.rollback'].includes(operation.kind)){
    result=await findCapabilityWorkflowRun(operation.id);if(!result)return operation;if(result.status==='completed'){const ok=result.conclusion==='success';if(operation.kind==='tests.run')status=ok?'passed':'failed';else if(operation.kind==='deploy.rollback')status=ok?'rolled_back':'failed';else status=ok?'succeeded':'failed';}else status='running';
  }else if(operation.kind==='git.pull-request'){
    result=operation.execution?.number?await getOperationPullRequest(operation.execution.number):await findOperationPullRequest(operation.plan.sourceRef);if(!result)return operation;status=result.merged?'merged':result.state==='closed'?'closed':'open';
  }else if(operation.kind==='repository.write'&&['executing','failed'].includes(operation.status)){
