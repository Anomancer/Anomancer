...
const STORE_KEY='archive/archive.json';
const STORE_ENV=['ANOMANCER_ARCHIVE_STORE'];
function mode(){return stateBackendMode(STORE_ENV);}
export function archiveStoreStatus(){const backend=stateBackendStatus(STORE_KEY,STORE_ENV);return{format:ARCHIVE_STORE_FORMAT,coreVersion:CORE_VERSION,...backend,maxObjects:MAX_OBJECTS,maxReceipts:MAX_RECEIPTS,automaticModelMemory:false,requiresContextGrant:true};}
export async function loadArchiveState({force=false}={}){const m=mode();if(m==='memory'){if(!memoryState)memoryState=defaultState();return clone(memoryState);}if(!force&&cache&&cache.expiresAt>Date.now())return clone(cache.state);const stored=await readStateJson(STORE_KEY,defaultState(),STORE_ENV),state=normalizeState(stored.value);cache={state,version:stored.version,expiresAt:Date.now()+CACHE_TTL_MS};return clone(state);}
async function mutate(fn,{expectedRevision=null}={}){const m=mode();if(m==='memory'){const current=await loadArchiveState({force:true});if(expectedRevision!==null&&Number(expectedRevision)!==current.revision)throw Object.assign(new Error('Arkisto muuttui toisessa istunnossa.'),{statusCode:409,code:'ARCHIVE_REVISION_CONFLICT'});const draft=normalizeState(await fn(clone(current))||current);draft.revision=current.revision+1;draft.updatedAt=now();memoryState=draft;return clone(draft);}for(let attempt=0;attempt<3;attempt++){cache=null;const current=await loadArchiveState({force:true}),version=cache?.version??null;if(expectedRevision!==null&&Number(expectedRevision)!==current.revision)throw Object.assign(new Error('Arkisto muuttui toisessa istunnossa.'),{statusCode:409,code:'ARCHIVE_REVISION_CONFLICT'});const draft=normalizeState(await fn(clone(current))||current);draft.revision=current.revision+1;draft.updatedAt=now();try{const written=await writeStateJson(STORE_KEY,draft,{expectedVersion:version,envNames:STORE_ENV});cache={state:draft,version:written.version,expiresAt:Date.now()+CACHE_TTL_MS};return clone(draft);}catch(error){if(error?.code==='STATE_WRITE_CONFLICT'&&attempt<2)continue;if(error?.code==='STATE_WRITE_CONFLICT')throw Object.assign(new Error('Archive Storen rinnakkaisuuskonflikti.'),{statusCode:409,code:'ARCHIVE_WRITE_CONFLICT'});throw error;}}throw Object.assign(new Error('Archive Storen rinnakkaisuuskonflikti.'),{statusCode:409,code:'ARCHIVE_WRITE_CONFLICT'});}
