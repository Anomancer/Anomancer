...
async function gh(workspaceId,path,options={}){
  const{token}=cfg(workspaceId),response=await fetch(`https://api.github.com${path}`,{...options,headers:{Accept:'application/vnd.github+json',Authorization:`Bearer ${token}`,'X-GitHub-Api-Version':'2022-11-28','User-Agent':'anomancer-operation-store-v1.18.4',...(options.headers||{})}}),text=await response.text();
