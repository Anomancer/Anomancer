...
export async function runArchiveCurator({workspaceId='',maxProposals=80,nowIso=''}={}){
  ...
  for(const [hash,group] of exactGroups){if(group.length<2)continue;duplicateGroups.push(group.map(x=>x.id));addProposal(proposals,{kind:'duplicate-exact',severity:'warn',title:'Täsmälleen sama sisältö useassa objektissa',message:`${group.length} arkisto-objektia jakaa saman content hashin ${hash.slice(0,12)}…`,objectIds:group.map(x=>x.id),workspaceId:group.every(x=>x.workspaceId===group[0].workspaceId)?group[0].workspaceId:'',projectId:group.every(x=>x.projectId===group[0].projectId)?group[0].projectId:'',recommendation:'Valitse ihmisenä pääversio. Muut voidaan myöhemmin merkitä historiallisiksi tai säilyttää provenance-syistä.'});}
