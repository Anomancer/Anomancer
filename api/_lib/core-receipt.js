...
export function createRunReceipt({contract,runtime=null,post,instruction='',result,meta={},toolPolicy=[],startedAt,finishedAt,orchestraRunId=null,orchestra=null,stageIndex=null,status='completed'}){
  ...
  return {
    format:RUN_RECEIPT_FORMAT,id,coreVersion:CORE_VERSION,orchestraRunId:orchestraRunId||null,orchestra:orchestra?{id:String(orchestra.id||''),name:String(orchestra.name||''),orchestraHash:String(orchestra.orchestraHash||'')}:null,stageIndex:Number.isInteger(stageIndex)?stageIndex:null,
