...
${JSON.stringify(outputs.critic)}`);
  ...
  async function runPipeline(){
    ...
    clearCheckpoint();let ctx;try{ctx=await createContext(initial,instruction.value.trim());}catch(error){return log(`Runtime Snapshot epäonnistui · ${error.message}`,'✗');}log(`Uusi ajo · workspace ${ctx.workspaceId} · editorin nykyinen tila · runtime rev ${ctx.runtimeRevision} jäädytetty serverillä. Vanha checkpoint poistettiin.`,'◇');
