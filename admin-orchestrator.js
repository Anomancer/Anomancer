...
if(desk){
  ...
  const FALLBACK_ORCHESTRA={id:'editorial',name:'Anomancer Toimitus',orchestraHash:'fallback',source:'built-in',steps:AGENT_FALLBACK.map((a,i)=>({id:`step-${String(i+1).padStart(2,'0')}`,mode:'sequential',agents:[a.id]}))};
  const EMPTY_ORCHESTRA={id:'',name:'Ei orkesteria',orchestraHash:'',source:'workspace-empty',steps:[]};
  ...
  const CHECKPOINT_PREFIX='anomancer.orchestra.checkpoint.v15.9.0';
  function workspaceId(){return runtime.service('workspaces')?.currentId?.()||'default';}
  function wsHeaders(extra={}){return runtime.service('workspaces')?.headers?.(extra)||{...extra,'X-Anomancer-Workspace':workspaceId()};}
  ...
  function emitTelemetry(code,detail={}){
    ...
    runtime.events.emit('telemetry',{code:String(code||'STATUS').replace(/[^A-Z0-9_]/g,'').slice(0,48),detail:safe});
  }
  ...
  function stageStateLabel(value=''){return({running:'käynnissä',done:'valmis',completed:'valmis',degraded:'heikentynyt',disabled:'pois käytöstä',error:'virhe',stopped:'pysäytetty'}[value]||'odottaa');}
  ...
  function currentIdentity(){const bridge=runtime.service('admin');const base=bridge?.getDraftIdentity?.()||{};return{path:String(base.path||''),sha:String(base.sha||''),instanceId:String(base.instanceId||''),fingerprint:fingerprint(),title:currentPost().title};}
...
${baseInstruction}`);
    ...
    if(stage==='package')bits.push('Tämä on orkesterin viimeinen koneellinen vaihe. Valmistele esitysmetadata ja vain varmennettujen supported-evidenssien luonnolliset inline-sijoitusehdotukset. Nykyinen Evidence Layer (claims + sources), teksti sekä ihmisen valitsema audience + audienceDepth ovat lukittuja eikä niitä saa kirjoittaa uusiksi. Palauta vain tarkka quote + anchorText + evidenceUrl. Älä väitä mitään julkaistuksi tai ihmisen hyväksymäksi.');
    return bits.join('\n\n').slice(0,12000);
  }
  async function callAgent(agent,post,custom,{orchestraRunId=null,stageIndex=null,runtimeSnapshotToken=''}={}){
    ...
    let r,d;try{r=await fetch('/api/admin/core?resource=agents',{method:'POST',credentials:'same-origin',signal:controller.signal,headers:wsHeaders({'Content-Type':'application/json','X-CSRF-Token':csrf}),body:JSON.stringify({agent,instruction:custom,post,orchestraRunId,stageIndex,runtimeSnapshotToken})});d=await r.json().catch(()=>({}));}finally{controllers.delete(controller);}
    if(!r.ok||!d.ok){
      if(d.policyDecision)await runtime.service('core')?.appendPolicyDecision?.(d.policyDecision);
  ...
  function stageSummary(stage,d){
    const m=d.meta||{},r=d.result||{};const bits=[];if(m.lengthRecovery)bits.push('tiivis tokenrajan varareitti');if(m.model)bits.push(m.model);if(m.outputTokens)bits.push(`${m.outputTokens} tok`);if(m.reasoningTokens)bits.push(`${m.reasoningTokens} päättelytokenia`);
    if(stage==='source'){
    ...
    if(stage==='package')bits.unshift(`metadata + ${Array.isArray(r.citationPlacements)?r.citationPlacements.length:0} viitesijoitusta`);return bits.join(' · ');
  }
  ...
  function checkpointPayload(ctx){return{
    version:'15.9.0',workspaceId:ctx.workspaceId||workspaceId(),orchestraRunId:ctx.orchestraRunId,orchestra:ctx.orchestra,runtimeSnapshotId:ctx.runtimeSnapshotId||'',runtimeSnapshotToken:ctx.runtimeSnapshotToken||'',runtimeRevision:Number(ctx.runtimeRevision||0),status:ctx.status||'running',draftIdentity:ctx.draftIdentity,initial:ctx.initial,post:ctx.post,outputs:ctx.outputs,metas:ctx.metas,runtimeProfiles:ctx.runtimeProfiles,stageStates:ctx.stageStates,nextIndex:ctx.nextIndex,failedIndex:ctx.failedIndex,failedError:ctx.failedError||null,receipts:Array.isArray(ctx.receipts)?ctx.receipts:[],baseInstruction:ctx.baseInstruction,startedAt:ctx.startedAt,finishedAt:ctx.finishedAt||'',terminal:terminal.textContent,humanApprovalRequired:true
  };}
  function saveCheckpoint(ctx){
    checkpoint=checkpointPayload(ctx);
    let persisted=true;try{sessionStorage.setItem(checkpointKey(),JSON.stringify(checkpoint));}catch{persisted=false;}
    updateCheckpointActions();
    if(!persisted)log('Selaimen tarkistuspisteen tallennus ei onnistunut. Ajo säilyy vain tässä välilehdessä.','⚠');
    return persisted;
  }
  function clearCheckpoint(){checkpoint=null;try{sessionStorage.removeItem(checkpointKey());}catch{}updateCheckpointActions();}
  function loadCheckpoint(){
    try{const raw=sessionStorage.getItem(checkpointKey());if(!raw)return null;const value=JSON.parse(raw);if(value?.version!=='15.9.0'||value?.workspaceId!==workspaceId()||!value?.post||!value?.outputs||!value?.draftIdentity||!value?.runtimeSnapshotToken)return null;return value;}catch{return null;}
  }
  ...
  function restoreCheckpoint(){
    ...
    terminal.textContent=cp.terminal||'$ tarkistuspiste palautettu';
  ...
  async function executeAgent(ctx,agentId,stepIndex,{autoRetry=true,postView=null,outputsView=null,metasView=null}={}){
    const stage=agentMeta(agentId),profile=ctx.runtimeProfiles?.[stage.id]||runtime.service('core')?.getRuntimeProfile?.(stage.id)||null;
    if(profile?.active===false){ctx.stageStates[stage.id]='disabled';ctx.metas[stage.id]={disabled:true,runtimeProfile:safeClone(profile)};stageState(stage.id,'disabled');log(`${stepIndex+1}/${STEPS.length} ${stage.label} ohitettiin · Coren ajoprofiili = POIS`,'○');emitTelemetry('STAGE_SKIPPED',{stage:stage.id,label:stage.label,step:stepIndex+1,stepCount:STEPS.length});return{disabled:true,runtime:profile,stage};}
    stageState(stage.id,'running');ctx.stageStates[stage.id]='running';log(`${stepIndex+1}/${STEPS.length} ${stage.label} käynnistyy · ajoprofiili ${Number(profile?.maxOutputTokens||0).toLocaleString('fi-FI')} tokenia`,'▶');emitTelemetry('STAGE_STARTED',{stage:stage.id,label:stage.label,step:stepIndex+1,stepCount:STEPS.length});
    ...
    while(true){
      ...
      try{
        ...
        if(d.runPersistence?.stored===false&&!d.runPersistence?.deferred)log(`${stage.label} valmis, mutta palvelimen ajokuitti jäi hetkeksi tallentumatta · ajo jatkuu eikä agenttitulosta hylätä`,'⚠');
        if(d.receipt?.id&&!ctx.receipts.some(r=>r.id===d.receipt.id))ctx.receipts.push(safeClone(d.receipt));
        try{await runtime.service('core')?.appendReceipt?.(d.receipt);}catch(error){log(`${stage.label} · paikallisen ajolokin tallennus epäonnistui (${error?.message||'tuntematon'}) · ajo jatkuu`,'⚠');}
  ...
  function beginUi({fresh=false}={}){
    running=true;stopRequested=false;finalRun=null;resultBox.hidden=true;applyBtn.hidden=true;runBtn.disabled=true;retryBtn.disabled=true;resumeBtn.disabled=true;stopBtn.disabled=false;instruction.disabled=true;runtime.service('admin')?.setEditorLocked?.(true);if(fresh){resetStages();terminal.textContent='';copyBtn.hidden=true;}setRunState('KÄYNNISSÄ');
  }
  function endUi(){running=false;controllers.clear();currentStageIndex=-1;runBtn.disabled=false;retryBtn.disabled=false;resumeBtn.disabled=false;stopBtn.disabled=true;instruction.disabled=false;runtime.service('admin')?.setEditorLocked?.(false);updateCheckpointActions();}
  ...
  async function failWithCheckpoint(ctx,index,error,{stopped=false}={}){
    ...
    await runStoreAction('checkpoint',ctx,{status:stopped?'stopped':'checkpoint',degradedStages:Object.entries(ctx.stageStates).filter(([,v])=>v==='degraded').map(([id])=>id),disabledStages:Object.entries(ctx.stageStates).filter(([,v])=>v==='disabled').map(([id])=>id),error:stopped?null:{code:error?.code||'',message:error?.message||''},receipts:ctx.receipts});
  }
  async function runRange(ctx,startIndex,endExclusive,{fresh=false,autoRetry=true,completeWhenDone=true}={}){
    ...
    try{
      ...
      if(completeWhenDone&&endExclusive>=STEPS.length)await finishRun(ctx);
        ...
        checkpoint=checkpointPayload(ctx);saveCheckpoint(ctx);setRunState('TARKISTUSPISTE / VALMIS');copyBtn.hidden=false;
        log(`Vaihe ${endExclusive}/${STEPS.length} valmis · tarkistuspiste päivitetty · Jatka tästä, kun haluat`,'◇');
      }
    }catch(error){
  ...
  async function resumePipeline(){
    if(running)return;const cp=checkpoint||loadCheckpoint();if(!cp)return log('Jatkettavaa tarkistuspistettä ei löytynyt.','✗');if(!await checkpointMatches(cp))return;const ctx=contextFromCheckpoint(cp);const start=Math.max(0,Math.min(STEPS.length,Number(cp.nextIndex)||0));
  ...
  async function retryFailedStage(){
    if(running)return;const cp=checkpoint||loadCheckpoint();if(!cp||!Number.isInteger(cp.failedIndex))return log('Uudelleen yritettävää epäonnistunutta vaihetta ei löytynyt.','✗');if(!await checkpointMatches(cp))return;const ctx=contextFromCheckpoint(cp);const index=cp.failedIndex;
...
Mitään ei julkaista tällä toiminnolla.`,confirmLabel:'Siirrä editoriin',destructive:changed}))return;
    ...
    if(q('#visualizations')&&Array.isArray(p.visualizations)){q('#visualizations').value=JSON.stringify(p.visualizations,null,2);fire(q('#visualizations'));}
    if(p.citationMode)qa('input[name="citationMode"]').forEach(x=>{x.checked=x.value===p.citationMode;});
    ...
    log('Lopputulos siirrettiin editoriin. Avataan seuraavaksi ihmisen julkaisun tarkistus; mitään ei julkaista ilman vahvistusta.','✓');emitTelemetry('HUMAN_APPLIED',{});setRunState('SOVELLETTU / JULKAISUN TARKISTUS');await runStoreAction('applied',{...finalRun,runtimeSnapshotToken:checkpoint?.runtimeSnapshotToken||'',orchestraRunId:finalRun.orchestraRunId});runtime.service('admin')?.openPublishReview?.();
  }
  function selectOrchestra(id){const found=ORCHESTRAS.find(item=>item.id===id)||ORCHESTRAS.find(item=>item.id==='editorial')||ORCHESTRAS[0]||EMPTY_ORCHESTRA;compileOrchestra(found);runBtn.disabled=!found.id;if(orchestraSelect&&orchestraSelect.value!==found.id)orchestraSelect.value=found.id;log(found.id?`Orkesteri valittu · ${found.name} · ${STEPS.length} vaihetta · ${PIPELINE.length} agenttia`:'Työtilalla ei ole orkesteria. Rakenna ensimmäinen Orkesterirekisterissä.','◈');}
  runtime.events.on('core-ready',event=>{const core=event.detail||{};AGENTS=(core.agents||AGENT_FALLBACK).map(a=>({id:a.id,label:a.label,role:a.role}));log('Mukautettujen orkestereiden tuki valmis','◈');});
  runtime.events.on('workspace-ready',event=>{if((event.detail?.template?.builtInOrchestraIds||[]).length)return;ORCHESTRAS=[];compileOrchestra(EMPTY_ORCHESTRA);runBtn.disabled=true;});
  runtime.events.on('orchestras-ready',event=>{const list=event.detail?.orchestras||[];ORCHESTRAS=list;const frozen=checkpoint?.orchestra;if(frozen&&list.some(item=>item.id===frozen.id)){compileOrchestra(frozen);runBtn.disabled=false;if(orchestraSelect)orchestraSelect.value=frozen.id;log(`Tarkistuspiste pitää orkesterin jäädytettynä · ${frozen.name||frozen.id}`,'◈');}else selectOrchestra(orchestraSelect?.value||'editorial');});
  runtime.events.on('workspace-change',event=>{if(running)return;finalRun=null;checkpoint=null;resultBox.hidden=true;applyBtn.hidden=true;copyBtn.hidden=true;ORCHESTRAS=[];compileOrchestra(EMPTY_ORCHESTRA);runBtn.disabled=true;restoreCheckpoint();log(`Työtila vaihdettu · ${event.detail?.workspace?.name||workspaceId()} · orkesterit ja ajoprofiilit ladataan tästä työtilasta`,'◇');});
  ...
  runtime.provide('orchestrator',{isRunning:()=>running,currentWorkspace:workspaceId,currentOrchestra:()=>ORCHESTRA,stop:()=>{if(running){stopRequested=true;for(const controller of controllers)controller.abort();}}});
