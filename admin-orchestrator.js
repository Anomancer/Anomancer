...
${JSON.stringify(outputs.critic)}`);
  ...
  window.addEventListener('anomancer:workspace-change',event=>{if(running)return;finalRun=null;checkpoint=null;resultBox.hidden=true;applyBtn.hidden=true;copyBtn.hidden=true;compileOrchestra(FALLBACK_ORCHESTRA);restoreCheckpoint();log(`Työtila vaihdettu · ${event.detail?.workspace?.name||workspaceId()} · orkesterit ja ajoprofiilit ladataan tästä työtilasta`,'◇');});
