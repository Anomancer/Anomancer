const CACHE_NAME = 'anomancer-lahetyskone-v1.18.7-p3';
...
self.addEventListener('activate', event => {
  event.waitUntil(
    caches.keys()
      .then(keys => Promise.all(keys.filter(key => key.startsWith('anomancer-lahetyskone-') && key !== CACHE_NAME).map(key => caches.delete(key))))
