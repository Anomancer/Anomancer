...
function sideEffectIntent(text){
  return /(?:julkaise\w*|deploy\w*|push\w*|lähetä\w*|poista\w*|delete\w*|(?:korjaa|päivitä)\w*\s+(?:(?:repo|koodi|tiedosto)\w*|[^\n]{0,120}\.(?:js|mjs|cjs|ts|tsx|jsx|json|css|html|sh)\b)|muuta\s+(?:repoa|tiedostoa|asetusta|koodia)|kirjoita\s+(?:verceliin|repoon|tiedostoon)|apply\s+patch|execute\w*|suorita\s+(?:muutos|patch|deploy|operaatio)\w*)/i.test(text);
}
