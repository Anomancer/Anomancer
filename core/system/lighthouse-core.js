...
function environmentSnapshot(environment={}){
  ...
  const appAllowed=lighthouseAppAllowed(environment);
  const explicitApp=String(environment.ANOMANCER_LIGHTHOUSE_APP||'').trim();
  const explicitLegacy=String(environment.ANOMANCER_LIGHTHOUSE_LAB||'').trim();
  ...
  return {
    ...
    explicitAppOverride:explicitApp||explicitLegacy||'',
    remoteDefaultLocked:['preview','production'].includes(vercelEnv)&&!appAllowed,
    productionDefaultLocked:vercelEnv==='production'&&!appAllowed
  };
}
...
export function createCoreSnapshot({
  ...
  return {
    ...
    authority:{
      ...
      note:'Lighthouse voi muodostaa ehdotuksia ja vastauksia, mutta sovellus ei anna mallille itsenäistä julkaisu- tai sivuvaikutusvaltaa.'
    },

    policy:{
      mode:'lighthouse-app',
      developmentStage:'vercel-direct',
    ...
    storage:{
      ...
      note:'Kevyen tilan paikalliset työtilat säilyvät tässä vaiheessa vain nykyisen selaimen tallennustilassa.'
    },
